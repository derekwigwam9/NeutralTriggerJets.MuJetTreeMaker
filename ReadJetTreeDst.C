...
// filepaths
static const TString iPath("pp200r12pt35u.withEvtFlags.r03a02rm1chrg.d29m11y2017.root");
static const TString oPath("pp200r12pt35u.withEvtFlags.r03a02rm1chrg.plots.d29m11y2017.root");
...
void ReadJetTreeDst(Bool_t inBatchMode=false) {
  ...
  JetTree -> SetBranchAddress("IsGoodTower", &IsGoodTower, &bIsGoodTwr);
  ...
  hJetArea[2]        = new TH1D("hJetAreaA", "Recoil jet area, all; A_{jet}; (1/N_{trg})dN_{jet}/dA_{jet}", nA, a1, a2);
  ...
  hJetEta[2]         = new TH1D("hJetEtaA", "Recoil jet #eta, all; #eta; (1/N_{trg})dN_{jet}/d#eta", nH, h1, h2);
  ...
  hAllDeltaPhi[2]    = new TH1D("hAllDeltaPhiA", "All jet #Delta#varphi, all; #Delta#varphi; (1/N_{trg})dN_{jet}/d#Delta#varphi", nDf, dF1, dF2);
  ...
  hJetDeltaPhi[2]    = new TH1D("hJetDeltaPhiA", "Recoil jet #Delta#varphi, all; #Delta#varphi; (1/N_{trg})dN_{jet}/d#Delta#varphi", nDf, dF1, dF2);
  ...
  hJetPtRaw[2]       = new TH1D("hJetPtRawA", "Recoil jet p_{T}^{raw}, all; p_{T}^{raw}; (1/N_{trg})dN_{jet}/dp_{T}^{raw}", nPt, pT1, pT2);
  ...
  hJetPtCorr[2]      = new TH1D("hJetPtCorrA", "Recoil jet p_{T}^{corr}, all; p_{T}^{corr}; (1/N_{trg})dN_{jet}/dp_{T}^{corr}", nPt, pT1, pT2);
  ...
  for (Int_t i = 0; i < nEvts; i++) {
    ...
    const Bool_t   isGoodTwr = IsGoodTower;
    const Bool_t   isGoodTrg = IsGoodTrigger;
    if (!isGoodRun || !isGoodEvt || !isGoodTwr || !isGoodTrg) continue;


    // determine triggers [0 for pi0, 1 for gamma-rich]
    Int_t  tspFlag    = -1;
    Bool_t inPi0Range = ((TSP >= MinPi0Tsp) && (TSP <= MaxPi0Tsp));
    Bool_t inGamRange = ((TSP >= MinGamTsp) && (TSP <= MaxGamTsp));
    if (inPi0Range)
      tspFlag = 0;
    else if (inGamRange)
      tspFlag = 1;
