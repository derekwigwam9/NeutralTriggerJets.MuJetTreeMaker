...
void CalculatePurity(const Bool_t isInBatchMode=false) {
  ...
  // io parameters
  const TString sOutput("pp200r9embed.purityExclusive.eTtrg920zt0306.d5m2y2018.root");
  const TString sInput[NFiles] = {"../../ThirdJetMaker/output/merged/pt5.thirdmaker.root", "../../ThirdJetMaker/output/merged/pt7.thirdmaker.root", "../../ThirdJetMaker/output/merged/pt9.thirdmaker.root", "../../ThirdJetMaker/output/merged/pt11.thirdmaker.root", "../../ThirdJetMaker/output/merged/pt15.thirdmaker.partialStatistics.root", "../../ThirdJetMaker/output/merged/pt25.thirdmaker.partialStatistics.root", "../../ThirdJetMaker/output/merged/pt35.thirdmaker.root"};
  ...
  const TString  sZtTrkMin("0.3");
  const TString  sZtTrkMax("0.6");
  ...
  if (!fOutput) {
    cerr << "PANIC: couldn't make output file!" << endl;
    return;
  }
  ...
  cout << "    Declared addresses and branches." << endl;
  ...
  // define trigger bins
  const Double_t eTtrgMin[NBins] = {9., 9., 11., 13., 15., 17., 19.};
  const Double_t eTtrgMax[NBins] = {20., 11., 13., 15., 17., 19., 20.};
  const Double_t eTtrgBin[NBins] = {9., 11., 13., 15., 17., 19., 20.};
  ...
  const TString sEtNames[NBins]  = {"920", "911", "1113", "1315", "1517", "1719", "1920"};
  ...
  for (UInt_t iFile = 0; iFile < NFiles; iFile++) {
    ...
    // open file
    TFile *fInput  = new TFile(sInput[iFile].Data(), "read");
    if (!fInput) {
      cerr << "PANIC: couldn't open input file no. " << iFile << "!" << endl;
      break;
    }
    cout << "    Processing file " << iFile + 1 << "/" << NFiles << "..." << endl;
    ...
    for (UInt_t iEvt = 0; iEvt < nEvts; iEvt++) {
      ...
      for (UInt_t iTrk = 0; iTrk < nTrks; iTrk++) {
        ...
        // fill histograms
        if (isInPi0cut) {
          hEtaTrk[0][0]     -> Fill(hTrk, weights[iWeight]);
          hPtTrk[0][0]      -> Fill(pTtrk, weights[iWeight]);
          hZtTrk[0][0]      -> Fill(zTtrk, weights[iWeight]);
          hEtaTrk[eTbin][0] -> Fill(hTrk, weights[iWeight]);
          hPtTrk[eTbin][0]  -> Fill(pTtrk, weights[iWeight]);
          hZtTrk[eTbin][0]  -> Fill(zTtrk, weights[iWeight]);
        }
        if (isInGamCut) {
          hEtaTrk[0][1]     -> Fill(hTrk, weights[iWeight]);
          hPtTrk[0][1]      -> Fill(pTtrk, weights[iWeight]);
          hZtTrk[0][1]      -> Fill(zTtrk, weights[iWeight]);
          hEtaTrk[eTbin][1] -> Fill(hTrk, weights[iWeight]);
          hPtTrk[eTbin][1]  -> Fill(pTtrk, weights[iWeight]);
          hZtTrk[eTbin][1]  -> Fill(zTtrk, weights[iWeight]);
        }
        ...
        if (isInZtCut) {
          if (isInPi0cut) {
            hDeltaPhi[0][0]     -> Fill(dFtrk, weights[iWeight]);
            hZtCalc[0][0]       -> Fill(zTtrk, weights[iWeight]);
            hDeltaPhi[eTbin][0] -> Fill(dFtrk, weights[iWeight]);
            hZtCalc[eTbin][0]   -> Fill(zTtrk, weights[iWeight]);
            if (isNearSide) {
              hDeltaPhiNS[0][0]     -> Fill(dFtrk, weights[iWeight]);
              hDeltaPhiNS[eTbin][0] -> Fill(dFtrk, weights[iWeight]);
            }  // end NS cut
          }  // end pi0 cut
          if (isInGamCut) {
            hDeltaPhi[0][1]     -> Fill(dFtrk, weights[iWeight]);
            hZtCalc[0][1]       -> Fill(zTtrk, weights[iWeight]);
            hDeltaPhi[eTbin][1] -> Fill(dFtrk, weights[iWeight]);
            hZtCalc[eTbin][1]   -> Fill(zTtrk, weights[iWeight]);
            if (isNearSide) {
              hDeltaPhiNS[0][1]     -> Fill(dFtrk, weights[iWeight]);
              hDeltaPhiNS[eTbin][1] -> Fill(dFtrk, weights[iWeight]);
            }  // end NS cut
          }  // end pi0 cut
        }  // end zT cut
    ...
    cout << "      Event loop finished in file " << iFile + 1 << ":\n"
         << "        " << nTrgBin[0][0] << " pi0 triggers\n"
         << "        " << nTrgBin[0][1] << " gamma triggers."
         << endl;

    for (UInt_t iBin = 0; iBin < NBins; iBin++) {
      for (UInt_t iTrg = 0; iTrg < NTrgs; iTrg++) {
        const Double_t nTrgScale = nTrgBin[iBin][iTrg] * weights[iWeight];
        nTrgTot[iBin][iTrg] += nTrgScale;
      }
    }
    cout << "      Scaled triggers." << endl;
  ...
  cout << "    File loop finished:\n"
       << "      " << nTrgTot[0][0] << " pi0 triggers in (weighted) total,\n"
       << "      " << nTrgTot[0][1] << " gamma triggers in (weighted) total."
       << endl;
   ...
   for (UInt_t iBin = 0; iBin < NBins; iBin++) {
     for (UInt_t iTrg = 0; iTrg < NTrgs; iTrg++) {
       ...
       const Double_t dFnorm = dFbin * nTrgTot[iBin][iTrg];
       const Double_t hNorm  = hBin * nTrgTot[iBin][iTrg];
       const Double_t pTnorm = pTbin * nTrgTot[iBin][iTrg];
       const Double_t zTnorm = zTbin * nTrgTot[iBin][iTrg];
  ...
  const Float_t dFrange[2] = {0., 2.03};
  const Float_t rRange[2]  = {0., 2.};
